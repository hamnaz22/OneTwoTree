...
from OutGroupSelectionContext import OutGroupSelectionContext
import operator
...
def init_cluster_db(cluster_context, context,outgroup_selection):
	...
	if not cluster_context.is_db_init:
		cache_flag = ploidb_config.getboolean('general', 'use_cache')
...
#			blast_sequences_into_db(cluster_context, context.db_file,
#								cluster_context.all_seqs_fasta_filename_no_multiple_accessions,
#								cluster_context.cluter_work_dir, cache_flag, context,outgroup_selection)
		else:
		...
		min_blast_results_outside_genus = int(ploidb_config['common outgroup']['min_blast_results_outside_genus'])
...
def prepare_blast_results_for_insert(blastResultFilename, blast_results_for_insert_filename, outgroup_seq_id, context):
	...
	with open(blastResultFilename, 'r') as in_file, open(blast_results_for_insert_filename, 'w') as out_file:
		...
		for line in in_file:
			line_words = line.rstrip().split(
				',')  # rstrip() remove the "\n" at the end, so that the last column won't be considered a string
			saccver = line_words[2]
			sseqid = line_words[0]
			...
			if saccver not in gbDict:
				...
				seq = gbDict[saccver]
				#organism = get_seq_organism(seq, synonym_dict)
				organism = get_seq_organism(seq, context.names_vs_accepted_dict)
				...
				if not " " in organism:
					...
					writer.writerow([outgroup_seq_id, genus, organism, gi, gb, line_words[1], saccver, line_words[3],
...
def create_view_for_cluster(db_file, cluster_context, outgroup_seq_id, fasta_filename, species_list):
	...
	with get_db_conn(db_file) as conn:
		...
		#update_is_seq_in_species_list_sql = "UPDATE blast_results SET is_seq_in_species_list = 1 where outgroup_seq_id = '%s' and lower(organism) like lower('%%%s%%')" % (
		#    outgroup_seq_id, genus_name)
		#curs.execute(update_is_seq_in_species_list_sql)
		...
		evalue_threshold = float(ploidb_config['common outgroup']['evalue_threshold'])
...
def init_cluster_blast_stats_per_attr(db_file, stat_summary_data, results_sql_clause, colname):
	...
	with get_db_conn(db_file) as conn:
		...
		low_precentile = float(ploidb_config['common outgroup']['low_precentile'])
...
def get_seqs_by_key(context,keys, new_organism=None):
	...
	#gbDict = get_indexed_seq_hash(ploidb_config['general']['genbankDir'])
...
def get_seqs_by_key_db(context,keys, new_organism=None):
	...
	blastDbPath = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['blastDbPath']
	out_seqs = list()
	for key in keys:
...
def get_filename_for_blast_cache(seq, genus_name):
	...
	path_for_cache = os.path.join(ploidb_config['general']['cache_path'], genus_name)
	filename_for_cache = os.path.join(path_for_cache, "bl_gi%s" % gi)
	return filename_for_cache

#This function is the original one, blast is performed vs the nt database at:
# #blastDbPath = /biodb/BLAST/Nucleotides/nt
def blast_sequences_into_db(cluster_context, db_file, fasta_filename, temp_results_dir, use_cached_results, context,outgroup_selection):
	blastBinPath = ploidb_config['general']['blastBinPath']
	blastDbPath = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['blastDbPath']
	...
	logger.debug("Using %s as identifier for this cluster", outgroup_seq_id)
	...
	if (not blast_result_already_in_db):
		...
		logger.debug("Executing prepare_blast_results_for_insert(%s, %s, %s, %s)" %
					 (blastResultFilename, blast_results_for_insert_filename, outgroup_seq_id, context))

		prepare_blast_results_for_insert(blastResultFilename, blast_results_for_insert_filename, outgroup_seq_id,
										 context)
...
#def check_and_add_cluster_NoOutgroup_to_concat(cid_to_check, ploidb_context, outgroup_context):
#	logger.debug("Checking the following cluster %s" % ploidb_context.get_cluster_by_id(cid_to_check))
#	current_outgroup_len = get_cluster_ids_total_seq_len(ploidb_context,outgroup_context.selected_cids_with_outgroup_seq)
...
#
#	max_concat_seq_length = int(ploidb_config['common outgroup']['max_concat_seq_length'])
#	min_species_ratio_for_cluster = float(ploidb_config['common outgroup']['min_species_ratio_for_cluster'])
#	new_to_existing_species_threshold_ratio = float(
#		ploidb_config['common outgroup']['new_to_existing_species_threshold_ratio'])
#	min_coverage = float(ploidb_config['common outgroup']['min_coverage'])
...
#			logger.debug(
#				"Since there are still candidate clusters with outgroup, the cluster will be kept in the list for future tries")
		...
		while all_cluster_length < max_concat_seq_length:# or i < num_of_clusters:
			...
			if 'Outgroup_User' in ploidb_context.UserFlags_dict:
				for taxID_str in added_species_dict[max_cluster_index]:
					logger.debug("Compare Tax %s to UserOutgroup tax %s" %(taxID_str,ploidb_context.UserOutgroupTaxId))
...
def Old_Write_Accession_Loci_Matrix(ploidb_context,matrix_dict_data,loci_desc_dict,specie_final_list):

	f_accession_clusters = open(ploidb_context.summary_accession_matrix_file,'w')
	f_accession_clusters.write('"Specie\Loci",')
	for loci_num in matrix_dict_data.keys():
		f_accession_clusters.write('"%s-%s",'%(loci_num,loci_desc_dict[loci_num]))
	f_accession_clusters.write('\n')
	for specie in specie_final_list:
		f_accession_clusters.write('"%s-%s",'%(specie,ploidb_context.species_names_by_tax_id[specie]))
		for loci_num in matrix_dict_data.keys():
			add_gap='yes'
			for item in matrix_dict_data[loci_num]:
				if specie in item:
					name=item.split('|')[0]
					taxID=item.split('|')[1]
					AccesionNum=item.split('|')[2]
					#remove_speciesName = item.replace('%s|'%specie,'')
					f_accession_clusters.write('"%s",'%AccesionNum)
					add_gap='No'
			if add_gap == 'yes':
				f_accession_clusters.write('"-",')
		f_accession_clusters.write('\n')
