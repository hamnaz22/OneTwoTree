...
# I patched this sql to the one below. In case where all the values are zero no min value is selected and this patch make sure that the value -200 will be selected
#  my $sql = "
...
#";
  ...
  my $sql = "
select COALESCE(min(evalue_exp),-200)
from $sst
where evalue_mant != 0
";
  my $minEvalueExp;
...
sub runSql {
  ...
  print LOGFILE "$sql $stepNumber \n";
  print LOGFILE "$skipPast \n";
