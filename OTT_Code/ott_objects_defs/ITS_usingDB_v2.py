...
#-------------------------------------------------------------------------------------------------------
def Filter_1st_Cluster_BC_combined(out_dir,BC_L,BC_S,log_f):
	...
	os.system(BC_cmd)
	log_f.write("Perform BlastClust on Combined file:")
	log_f.write(BC_cmd)
	cluster_number=0
	...
	#Copy only accessions in cluster 1
	log_f.write("Clusters Data:")
	for cluster_i in range(1,cluster_number+1):
		log_f.write(str(len(Cluster_data_AccList[cluster_i])))
		for item in Cluster_data_AccList[cluster_i]:
			log_f.write(str(item) + ',')
		log_f.write('\nCluster#%d: %d taxa, %f%%\n' %(cluster_i,Cluster_TaxaCount_dict[cluster_i],(Cluster_TaxaCount_dict[cluster_i]/total_taxa_count)*100))
		log_f.write('\n')

	#remove the accessions from the fasta file:
	remove_accessions_from_fasta(Cluster_data_AccList[1],itscombfasta)

	#Afetr cpmbined was filtered run BlustClust
	fasta_Its1_comb = out_dir+'/combined_its1_forBC.fasta'
	BC_out_ITS1_comb_file = out_dir+'/Comb_ITS1_BCOutput.txt'
	its1_taxa_list = combine_2_fasta(its1fasta,itscombfasta,fasta_Its1_comb,log_f)
	fasta_Its2_comb = out_dir+'/combined_its2_forBC.fasta'
	BC_out_ITS2_comb_file = out_dir+'/Comb_ITS2_BCOutput.txt'
	its2_taxa_list = combine_2_fasta(its2fasta,itscombfasta,fasta_Its2_comb,log_f)
	...
	BC_cmd = 'blastclust -i %s -o %s -p F -L %f -b F -S %d -e F' %(fasta_Its1_comb,BC_out_ITS1_comb_file,BC_L,BC_S)
	os.system(BC_cmd)
	BC_cmd = 'blastclust -i %s -o %s -p F -L %f -b F -S %d -e F' %(fasta_Its2_comb,BC_out_ITS2_comb_file,BC_L,BC_S)
	os.system(BC_cmd)

	#Copy for taxa in cluster 1:
	taxa_list_ITS1_clst = retrun_taxa_Clst(BC_out_ITS1_comb_file,1)
	taxa_list_ITS2_clst = retrun_taxa_Clst(BC_out_ITS2_comb_file,1)
	if 'No Cluster' not in taxa_list_ITS1_clst and 'No Cluster' not in taxa_list_ITS2_clst:
		ITS1_in_BC_res = set(taxa_list_ITS1_clst) - set(its1_taxa_list)
		log_f.write('ITS1_in_BC_res:\n')
		for item in ITS1_in_BC_res:
			log_f.write(item+',')

	#Remove from ITS1 and ITS2 only files taxa that were not included in the largest BC cluster:
	remove_accessions_from_fasta(taxa_list_ITS1_clst, its1fasta)
	remove_accessions_from_fasta(taxa_list_ITS2_clst, its2fasta)

	# !!! need to check at what cutoff we need to add mroe clusters
...
def retrun_taxa_Clst(BC_result_file,cluster_num):
	...
	cluster_number = 0
	Cluster_data_spcNum=0	#number of taxa in cluster
	accession_list=[]
	with open(BC_result_file, 'r') as BCres_f:
		...
		for line in BCres_f:
			...
			if cluster_num == cluster_number:	#The cluster number we would like to save
				data_spl = line.strip().split(" ")
				Cluster_data_spcNum = len(data_spl)
				for item in data_spl:
					item_spl = item.split('|')
					accession_list.append(item_spl[1])
...
#-------------------------------------------------------------------------------------------------------
def remove_accessions_from_fasta(aaccession_list,fasta_file):

	shutil.copyfile(fasta_file,fasta_file+'_original')
	seqs = list(SeqIO.parse(fasta_file+'_original', "fasta"))
	fasta_filtered_f = open(fasta_file, 'w')
	for seq in seqs:
...
def combine_2_fasta(fasta1,fasta2,fasta_1_2,log_f):
	...
	copy_seqs_list=[]
	return_taxa_list = []
	seqs1 = list(SeqIO.parse(fasta1, "fasta"))
	for seq in seqs1:
		return_taxa_list.append(seq.description.split('|')[1])
		copy_seqs_list.append(seq)
	write_to_fasta(fasta_1_2, copy_seqs_list)

	copy_seqs_list=[]
	seqs2 = list(SeqIO.parse(fasta2, "fasta"))
	for seq in seqs2:
		copy_seqs_list.append(seq)
	write_to_fasta(fasta_1_2, copy_seqs_list)

	log_f.write('!!!Inside function "combine_2_fasta" with fasta1 %s, fasta2 %s fasta_1_2 %s\n\n' %(fasta1,fasta2,fasta_1_2))
	log_f.write(str(len(seqs1))+'\n')
	log_f.write(str(len(seqs2))+'\n')
...
def main_ITS_py(context,OutDir, fasta, gene_db, scriptsDir, configFile, guidanceFlag, msa_software):
	...
	BC_L = 0.3
	BC_S = 60
	Filter_1st_Cluster_BC_combined(OutDir, BC_L, BC_S,f_mainITS_log)
