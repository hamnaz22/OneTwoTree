...
#-------------------------------------------------------------------------------------------------------
def Filter_1st_Cluster_BC_combined(out_dir,BC_L,BC_S,log_f,TotalTaxaNum):
	...
	for cluster_num in Cluster_data_AccList.keys():
		...
		if cluster_num == 1:
			...
			its_clusters_list.append(out_dir)
	...
	# !!! need to check at what cutoff we need to add more clusters
...
def main_ITS_py(context,OutDir, fasta, gene_db, scriptsDir, configFile, guidanceFlag, msa_software,Clst_Dir):
	...
	if Clst_Dir == "1":
		...
		its_clusters_list = Filter_1st_Cluster_BC_combined(OutDir, BC_L, BC_S,f_mainITS_log,TotalTaxaNum)
		f_mainITS_log.write("ITS Clusters:\n")
		for its_clust in its_clusters_list:
			f_mainITS_log.write("%s\n" %its_clust)
	...
	if Clst_Dir == "1":
		its_clusters_list.append(OutDir)
		context.its_cluster_list = list(its_clusters_list)
