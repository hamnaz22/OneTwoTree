...
#a version of "orthomclAdjustFasta, where the output directory is given as a parameter
...
my $inputfile = $ARGV[1];
my $idField = $ARGV[2];
my $compliantFastaDir = $ARGV[3];

open(IN, $inputfile) || die "Can't open input file '$inputfile'\n";
open(OUT, ">$compliantFastaDir/$taxoncode.fasta") || die "Can't open output file '$compliantFastaDir/$taxoncode.fasta'\n";
...
while(<IN>) {
  if (/\>/) {
    ...
    s/\s*\|\s*/\|/g;
    my @a = split(/[\s\|]/);
    print OUT ">$taxoncode|$a[$idField-1]\n";
  } else {
    print OUT $_;
  }
}
