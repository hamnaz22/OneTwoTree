...
from ClusterContext import ClusterContext, SeqType
from ItsClusterContext import ItsClusterContext
from ConcatClusterContext import ConcatClusterContext
...
class PloiDbContext:
    ...
    # Removed since it's not in use (by md)
    #def get_gb_seqs_dict(self):
    #    if self.gb_seqs_dict is None:
    #        self.gb_seqs_dict = get_indexed_seq_hash(ploidb_config['general']['genbankDir'])
#
    ...
    # def get_cluster_contexts_for_concat_tree_candidates(self):
    # 	return cluster_contexts_for_concat_tree_candidates
    # 	return self.get_clusters_by_ids(self.cluster_contexts_for_concat_tree_candidate_ids,self.cluster_contexts)
    ...
    def get_clusters_by_type(self, cluster_type, clusters):
        logger.debug("MDebug Inside get_clusters_by_type - cluster_type:")
        logger.debug(cluster_type)
        ...
        for cc in clusters:
            if cc.cluster_type in cluster_type:
                logger.debug("MDebug inside for cc loop - cc.cluster_type:")
                logger.debug(cc.cluster_type)
    ...
    def get_clusters_contexts(self, clusters):
        ...
        #cluster_contexts_by_type_list = list()
        #for cc in clusters:
        #    logger.debug("## cluster id: %d,  Type: %s " %(cc.cluster_id, cc.cluster_type))
        #    cluster_contexts_by_type_list.append(cc)
        #logger.debug("Found a total for %d clusters" % (len(cluster_contexts_by_type_list)))
        #return cluster_contexts_by_type_list
    ...
    #def init_cluster_contexts_for_loci_trees_list_bak(self):
    #	ordered_clusters = sorted(self.cluster_contexts, key=lambda cluster: cluster.get_data_matrix_size(estimated=True), reverse=True)
    #	for cluster_context in ordered_clusters:
    #		ratio_to_largest = (1.0 * cluster_context.get_data_matrix_size(estimated=True)) / ordered_clusters[0].get_data_matrix_size(estimated=True)
    #		if ratio_to_largest >= 0.25 and len(self.cluster_contexts_for_loci_trees) <= self.MAX_NUMBER_OF_LOCUS_TREES:
    #			init_cluster_db(cluster_context, self)
    #			if cluster_context.locus_tree_outgroup_selection.outgroup_found:
    #				logger.info("Cluster %s will be added to list of contexts for locus tree. ratio is %f" % (cluster_context,ratio_to_largest))
    #				self.cluster_contexts_for_loci_trees.append(cluster_context)
    #			else:
    #				logger.info("Cluster %s - %s has no outgroup and will be skipped" % (cluster_context.cluster_id,cluster_context))
    #
    #	for cluster in self.cluster_contexts_for_loci_trees:
    #		cluster.is_used_for_locus_tree = True
    #
    #	no_of_locus_trees = len(self.cluster_contexts_for_loci_trees)
    #	ratio_of_selected_clusters = 1.0 * no_of_locus_trees / len(self.cluster_contexts)
    #	if (no_of_locus_trees <= self.MAX_NUMBER_OF_LOCUS_TREES / 2) and  ratio_of_selected_clusters < 0.5:
    #		logger.error("Only %d clusters out of %d will be used for locus tree creation" % (no_of_locus_trees,len(self.cluster_contexts)))
