...
from handleMultAccessions import get_seq_with_max_average_blast_score
from NR_NameMatch import do_resolve_names
import copy
...
#write_standarize_fasta(context.largeSynToAdd_file, LargeTaxon_edited_seqs)
	...
	for seq in syn_of_large_seqs:
		...
		with open(large_fasta_file,'a') as f_toWrite:
			...
			logger.info("Large taxon %s was added with sequences of a synonym" %taxId)
...
def call_Check_Jmt_Done(jobs_dirs_list):
	...
	jmt_polling_time = ploidb_config['general']['JMT_PollingTime']
...
def remove_shortLong_seqs(blast_results, db_fasta_file, path_of_selected_seq, blast_FilterOutput,log_file):
	...
	script_path = ploidb_config['general']['OTT_MAIN'] + 'ott_scripts/'
...
def extract_names_from_rows(context,rows_db,db_name):
	...
	# Prepare Synonym Data: if Rank is species than Accepted is same taxid else parent taxid
	# Only in case of name resolution:
	...
	#if context.UserFlags_dict['NameResType'] == 'SpellingCorrection' or context.UserFlags_dict['NameResType'] == 'MatchedNames':
	#	Acc_TaxId_for_Synonym=dict()
	#	Acc_Name_for_Synonym=dict()
	#	for synTaxID in synTaxID_List:
	#		#Here we only set the TaxId of the Accepted name according to the following:
	#		#If the syn name is ranked as species than the Accepted is the scientific with the same TaxId
	#		#If is sub or var than the Accepted TaxId is the Parent TaxId
	#		#The names will be set after and they will be ONLY scientific names !!!
	#		if context.synTaxId_rank_dict[str(synTaxID)] == 'species':              #TBD need to check if not Name Rank and not TaxId rank ?????
	#			Acc_TaxId_for_Synonym[synTaxID]=synTaxID
	#			context.syn_acc_TaxID_dict[synTaxID]=Acc_TaxId_for_Synonym[synTaxID]
	#		elif context.synTaxId_rank_dict[str(synTaxID)] in ['subspecies','varietas']:
	#			Acc_TaxId_for_Synonym[synTaxID]=context.synTaxId_Parent_dict[synTaxID]
	#			context.syn_acc_TaxID_dict[synTaxID]=Acc_TaxId_for_Synonym[synTaxID]
	#			context.parent_species_tax_id_by_subsp_tax_id[synTaxID] = context.TaxId_Parent_dict[str(synTaxID)]
	#			context.parent_species_names_by_parent_tax_id[context.TaxId_Parent_dict[str(synTaxID)]] = context.TaxId_name_dict[context.TaxId_Parent_dict[str(synTaxID)]]
#
#
	#	logger.debug("-- SYNONYM DATA: STRT    @@@@@@@@@@@@@@@@@@")
	#	logger.debug(context.syn_acc_TaxID_dict)
	#	#Get scientific names for all Accepted TaxIds:
	#	Synonym_TaxId_queryStr=turn_list_to_DB_list(context.syn_acc_TaxID_dict.values())
	#	query_acc_names = "select TaxID,Name from ncbi_names_db where TaxID IN (%s) and Type is 'scientific name'" %Synonym_TaxId_queryStr
	#	logger.debug("Synonym select query: ")
	#	logger.debug(query_acc_names)
	#	db_rows = query_ncbi_db(query_acc_names,db_name)
	#	for row in db_rows:
	#		TaxID=str(row[0])
	#		Name=row[1].replace(",","").replace("'","")
	#		context.syn_acc_dict[TaxID]=Name
	#	logger.debug(context.syn_acc_dict)
	#	logger.debug("-- SYNONYM DATA: END    @@@@@@@@@@@@@@@@@@")
...
def db_name_matching(db_name,Names_input_list,context):
	...
	nr_match_cmd = "python " + context.cluster_script_dir + "/NR_NameMatch.py \
	--db-filename %s \
	--input-filename %s \
	--results-filename %s \
	--log-filename %s \
	...
	if context.UserFlags_dict['NR_DB_name'] == 'PLT':
		db_file = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['TPL_ALL_DB']
		...
		db_file = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['COL_NAMES_DB']
		list_with_syn_list,syn_vs_acc_dict = list_added_syn_acc(context,db_file,accepted_ids_list)
		logger.debug(list_with_syn_list)
		logger.debug(syn_vs_acc_dict)
		return list_with_syn_list,syn_vs_acc_dict
...
def return_species_list_debug(input_list,context):
	...
	if context.UserFlags_dict['NR_DB_name'] == 'NCBI': #default
		...
		db_name = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['TPL_ALL_DB']
	elif context.UserFlags_dict['NR_DB_name'] == 'COL': # the plant list
		db_name = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['COL_NAMES_DB']
	else:
	...
	if context.UserFlags_dict['Filter_SubSp'] == 'on' or context.UserFlags_dict['Filter_Hybrids'] == 'on' or context.UserFlags_dict['Filter_Unresolved'] == 'on':
		...
		for taxId in TaxId_rank_dict.keys():
			...
			if context.UserFlags_dict['Filter_SubSp'] == 'on':
				if rank == 'varietas' or rank == 'subspecies':
