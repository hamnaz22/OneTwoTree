...
my $branch_length_model = $ARGV[14];
#my $node_date_txt = "None";
#my $split_max_age = "None";
#my $split_name1 = "None";
#my $split_name2 = "None";
print("@ARGV\n");
#if ( @ARGV > 15){
#	$node_date_txt = $ARGV[15];
#	$split_max_age = $ARGV[16];
#	$split_name1 = $ARGV[17];
#	$split_name2 = $ARGV[18];
#}
...
#MD#foreach my $p ("nruns","nchains","ngen","samplefreq","diagnfreq","mcmcdiagn","relburnin","burninfrac", "Stoprule","Stopval","Checkpoint","Checkfreq"){
#	$mcmcString .= $p."=".$paramHash{$p}." ";
#}
my $sumtString = "";
...
#Check if Node dating and add block:
#if ( @ARGV > 15){
#	print CONF "$node_date_txt\n";
...
# Convert input MSA to PHYLIP format, using the convertToPhylipMSAFormat.pl script

	#MD my $phyName = "$clusterName" . "_phy.phy";    # make a name for the phy file
	#MD system( 'perl',
	#MD 	'/groups/itay_mayrose/shiranabad/Maya/ploiDB/SVN/mrbayes/convert_fasta2phylip.pl',
	#MD 	$msaFile, "$path/$phyName" ) ==
	#MD   0    # run script with its arguments (input and output)
	#MD   or die "Can't convert MSA file to PHYLIP !";

# Run the MrAIC script on the PHYLIP file
	#MD system( 'perl', '/groups/itay_mayrose/shiranabad/Maya/ploiDB/SVN/mrbayes/mraic.pl',
	#MD "$msaFile\_codedNames_phy.phy" ) == 0
	#MD  or die "Can't run MrAIC !";
