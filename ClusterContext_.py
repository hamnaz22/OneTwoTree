...
import re
import operator
from OutGroupSelection import OutGroupSelection
...
#
	...
	def init_cluster_type(self):
		...
		logger.debug("MDebug: fasta file for init_cluster_type -> %s" %self.all_seqs_fasta_filename)
		logger.debug("About to iterate %d seqs" % len(all_seqs))
		genes_list=[]
		for seq in all_seqs:
			one_gene = ''
			no_brack=''
