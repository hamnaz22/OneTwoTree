...
from ploidbCommon import getSpeciesSequenceFileLocation, logger, get_ordered_seqs, writeSequenceInFastaFormat
from ploidbUtils import turn_list_to_DB_list
import codecs
...
def DB_getGBsequences(context, fasta_filename, gb_filename, nextTaxId):
	...
	genbank_db = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['GENBANK_DB']
	max_seq_length = int(ploidb_config['general']['max_seq_length'])
	hugeTree_flag = ploidb_config['general']['Huge_trees']
	grp_list = ploidb_config['general']['GENBANK_GRP_LIST'].split(',')
		...
		while df_count < df_length:
			...
			if length < max_seq_length:
				...
				if hugeTree_flag and no_of_seq_for_taxonid >= 1000:
					...
					logger.info("HugeTree taxid %s exceeds Max number of seqs 1000" % (taxon))
	...
	for Seq_file in files:
		...
		if fasta_filename is not None:
			...
			#remove input files:
			#if os.path.isfile(context.fasta_cdhit_temp_input_f):
			#	os.remove(context.fasta_cdhit_temp_input_f)
			#if os.path.isfile(context.fasta_cdhit_temp_output_f):
			#	os.system(context.fasta_cdhit_temp_output_f)
...
def Create_names_forDBselect(names_list):
	species_list=list()
...
def Create_taxIds_forInQuery(taxId_list):
	taxa_list=list()
	for taxId in taxId_list:
		taxId.strip()
		new_taxId = "'"+taxId+"'"
		taxa_list.append(new_taxId)
	taxId_list_str = ','.join(taxa_list)
	return taxId_list_str

def CreateTPLdb_query(taxon_name,context):

	Genus_flag=0
	if 1 != len(taxon_name.split()):    # -> species name
		#SELECT Species FROM All_TPL WHERE Genus IN ('Asparagus','Solanus')
		species_list_db = Create_ListofSpecies(context)
		logger.debug(species_list_db)
		print("=============================================")
		print("List of species CASE - FOR DATABASE")
		print(species_list_db)
		print("=============================================")
		query_line = "SELECT * FROM names_vs_tax_ids WHERE Name IN (%s)" % species_list_db
		#query_line = "SELECT Id, Original_name, Name, Score, Coded_Name FROM NCBI_AfterNameRes WHERE Name IN (%s)" % species_list_db
		#query_line_syn = "SELECT Name, Accepted_name FROM tpl_names WHERE Accepted_name IN (%s)" % species_list_db
		query_line_syn = 'None'
	elif taxon_name.isdigit():
		query_line = "SELECT * FROM names_vs_tax_ids WHERE TaxID LIKE '%s'" % taxon_name
		query_line_syn = 'None'
	else:    # Genus name
		#query_line = "SELECT Id, Original_name, Name, Score, Coded_Name FROM NCBI_AfterNameRes WHERE Name IN "\
		#			"(SELECT Name FROM tpl_names WHERE Accepted_name LIKE '%s%%'"\
		#			"UNION ALL "\
		#			"SELECT Accepted_name FROM out_of_genus WHERE Accepted_Genus LIKE '%s')" % (taxon_name,taxon_name)
		query_line = "SELECT * FROM names_vs_tax_ids WHERE Name LIKE '%s %%'" % taxon_name
		query_line_syn = 'None'
		Genus_flag=1
	return query_line,query_line_syn,Genus_flag


#Yoni_original_starts_here:
#=====================================
##    term = taxon_name + "[Subtree](varietas[rank] OR subspecies[rank] OR species[rank])"
##
##    if should_ignore_subsp or should_merge_subsp:
##        term = taxon_name + "[Subtree](species[rank])"
##
##    db_name = "taxonomy"
##    max_number_of_species = 20000
##
##    # checking cur taxon species list is not to large
##    number_of_species = get_results_count_from_taxonomy_db(term)
##    if int(number_of_species) > max_number_of_species:
##        logger.error("too many results for " + taxon_name + "(" + str(number_of_species) + "). Skipping")
##        return list(), list()
##    else:
##        # calling Entrez for id's subtree of taxon_name. using history for next call
##        handle = Entrez.esearch(db=db_name, term=term, retmax=number_of_species, usehistory="y")
...
##        else:
##            # building id_list of species for next query
##            id_list_string = ','.join(id_list)
##
##            # calling entrez for name of each species
##            handle = Entrez.efetch(db=db_name, id=id_list_string, webenv=webenv, query_key=query_key)
...
##                tax_id = record["TaxId"]
##                species_names_by_tax_id[tax_id] = record["ScientificName"]
##
##
##            # adding supsp. and var. if merge is required
##            if should_merge_subsp:
##                # calling Entrez for id's of subsp. and var. under taxon_name. using history for next call
##                term = taxon_name + "[Subtree](varietas[rank] OR subspecies[rank])"
##                handle = Entrez.esearch(db=db_name, term=term, retmax=number_of_species, usehistory="y")
##                record = Entrez.read(handle)
##                handle.close()
##                webenv = record["WebEnv"]
...
##                    id_list_string = ','.join(id_list)
##
##                    # calling entrez for data on each subsp. and var.
##                    handle = Entrez.efetch(db=db_name, id=id_list_string, webenv=webenv, query_key=query_key)
##                    records = Entrez.read(handle)
##
##                    for record in records:
...
##                        species_names_by_tax_id[tax_id] = record["ScientificName"]
##
##                        parent_tax_id = record["ParentTaxId"]
##                        parent_name = species_names_by_tax_id[parent_tax_id]
##                        parent_species_names_by_subsp_tax_id[tax_id] = parent_name
##                        parent_species_tax_id_by_subsp_tax_id[tax_id] = parent_tax_id
##
##            return list(species_names_by_tax_id.keys()), list(
##                species_names_by_tax_id.values()), parent_species_tax_id_by_subsp_tax_id, parent_species_names_by_subsp_tax_id
##

def get_results_count_from_taxonomy_db(term):
	db_name = "taxonomy"
	...
	for row in record["eGQueryResult"]:
		if row["DbName"] == db_name:
			logger.info("Found " + row["Count"] + " results for db=" + db_name + " term=" + term)
...
def get_taxa_sequences(context,species_ids, fasta_filename, gb_filename, p):
	...
	if DB_StandAlone == 0:
		for taxon_id in species_ids:
			...
			CDHIT_FLAG = ploidb_config['general']['cd_hit_flag']
			print(CDHIT_FLAG)
			if CDHIT_FLAG == 'FALSE':
