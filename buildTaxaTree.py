...
#from name_resolve import do_resolve_names  #Name resolution-> need to check if updated !!!
...
def perform_ML_UltrametricNoDate(context,ml_tree_method,tree_file_path):
	...
	if ml_tree_method == 'treepl':
		...
		scripts_dir = ploidb_config['general']['OTT_MAIN'] + 'ott_scripts/'
		r_path = ploidb_config['diff_soft']['R_PATH']
	...
	if ml_tree_method == 'dpp':
		cmd_choose_one = ploidb_config['diff_soft']['TREE_ANNO'] + ' -heights mean %s.ant.tre outfile.ant.tre.out.treeannotator.tre' %(context.xml_dir+'/UltrametricTree.tree')
...
def calc_merged_clusterData_old(context, cluster_id, line_origin):
	#"ClusterID,Desc,Type,length,SpeciesCnt,SpeciesIds,IncludedInConcat\n")

	species_names = []
	split_line = line_origin.split(',')
	logger.debug("split_line:")
	logger.debug(split_line)
	desc_origin = split_line[1]
	type_origin = split_line[2]
	median_origin = split_line[3]
	species_cnt_origin = split_line[4].replace('"', '')
		...
		if 'cluster_' in key:
			if context.UserFlags_dict[key] == 'on':
				...
				for clustType in list_to_include:
					if os.path.exists(context.working_dir + '/' + str(cluster_index) + '/' + clustType):
	...
	if (f_end_status_name == context.debug_dir + '/AlignEnd_FinishedExecutionWithConcatFile'):
		...
		if os.path.exists(fastaFilesList):
			if context.UserFlag_mbUserModel != 'None':
				mb_UserModel_part12(context.working_dir, context.cluster_script_dir, fastaFilesList,
									context.UserFlag_mbUserModel)
			else:
				JMTjobs_dirs_list = part1_create_jmt(context.working_dir, context.cluster_script_dir, fastaFilesList, context.standAlone_Flag)
		else:
			logger.info(
				"=======================================================================================================")
			logger.info(
		...
		if os.path.exists(TreeFile):
			...
			if nodeDateTxt != 'Empty':
				...
				#				  context.UserFlags_dict['MinAge'],context.UserFlags_dict['MaxAge'],
			...
			create_mb_config_file(genus, context.working_dir, fastaFilesList, mb_params_list,
								  context.cluster_script_dir, clock_model_param, relaxed_branch_option_param,split_flag)
...
def concat_alignment_rerun(context):
	...
	with open(indexFileName, "w") as handle:
		for line in f_origin_concat_list:
			...
			if m:
				...
				#		logger.info("cluster_context id is : %s" % cluster_context.cluster_id)
				#		logger.info("List of concat clusters is:")
				#		logger.info(context.get_concat_clusters())
				#		if cluster_context.cluster_id in context.get_concat_clusters():
				#			logger.info("INSIDE concat_alignment concat_alignment concat_alignment concat_alignment")
				#			logger.info(cluster_context)
				#			#logger.info(cluster_context.cluster_id)
				#			aligned_fasta_filname = cluster_context.all_seqs_for_tree_creation_fasta_filename
	...
	concatAlignPAth = ploidb_config['general']['OTT_MAIN']
...
def RERUN_concat_alignment_SingleOutgroup(context):
	dict_clusterMode = {0: "Don't Include Clusters", 1: "Use NonConcat Clusters", 2: 'Use Concat Clusters'}
	logger.info("Starting concatenation")
	create_dir_if_not_exists(context.mrbayes_concat_work_dir)

	#Create rerun dictionary for clusters type:
	cluster_id_Type_dict = Create_ClusterType_dict(context)
	NucClusterMode = context.Nuc_clusterMode
	MtClusterMode = context.Mt_clusterMode
	ChloroClusterMode = context.Chloro_clusterMode
	...
	# cluster_context.all_seqs_for_concat_tree_creation_fasta_filename   ----->>>>   self.cluster_concat_work_dir + "/seqs-organism-concat.fasta
	indexFileName = context.fasta_files_list_to_concat
	with open(indexFileName, "w") as handle:
		#This section will decide which clusters to concat: default is to use all clusters separately:
		for idx in context.rerun_chosen_clusters:
			aligned_fasta_filname = context.mrbayes_concat_work_dir + '/' + str(idx) + '/seqs-organism-concat.fasta'
			if cluster_id_Type_dict[idx] == 'NUC' and NucClusterMode == 1:
				logger.debug("Writing %s to %s" % (aligned_fasta_filname, indexFileName))
				handle.write(aligned_fasta_filname + "\n")

			if cluster_id_Type_dict[idx] == 'mtDNA' and MtClusterMode == 1:
				logger.debug("Writing %s to %s" % (aligned_fasta_filname, indexFileName))
				handle.write(aligned_fasta_filname + "\n")

			if cluster_id_Type_dict[idx] == 'cpDNA' and ChloroClusterMode == 1:
				logger.debug("Writing %s to %s" % (aligned_fasta_filname, indexFileName))
				handle.write(aligned_fasta_filname + "\n")
...
def UpdateContextFlags(context):
	...
	#context.UserFlags_dict['concatAll_Nuc'] = 'off'
	#context.UserFlags_dict['concatAll_Mt'] = 'off'
	#context.UserFlags_dict['concatAll_CP'] = 'off'
	context.Nuc_clusterMode = calc_clustMethod(context.UserFlags_dict['include_Nuc'])
	context.Mt_clusterMode = calc_clustMethod(context.UserFlags_dict['include_Mt'])
	context.Chloro_clusterMode = calc_clustMethod(context.UserFlags_dict['include_CP'])
	...
	#Node Dating data:
	#if context.UserFlags_dict['SplitDiv']:
	#	f_params_web.write('Divergence time estimation:\n')
	#	f_params_web.write('Split: ' + context.UserFlags_dict["SplitDiv"] + '\n')
	#	f_params_web.write('Min Age: ' + context.UserFlags_dict["MinAge"] + '\n')
	#	f_params_web.write('Max Age: ' + context.UserFlags_dict["MaxAge"] + '\n')
	return
...
def cluster_sequences_file_with_orthomcl(fasta_seq_filename, id, scripts_path, gb_seq_filename, output_dir,
	...
	if context.UserFlags_dict['ClusteringMethod'] == 'Ortho':
		...
		orthomcl_bin_path = ploidb_config['orthomcl']['orthomcl_bin']
...
def cluster_ITS(context, fasta_seq_filename, scripts_path, gb_seq_filename):
	...
	min_species_in_cluster = int(ploidb_config['general']['min_species_in_cluster'])
	list_its_taxas=[]
...
def drop_orthomcl_scehma(genus_taxon_id):
	...
	db_host = ploidb_config['ploidb_mysql']['hostname']
	db_user = ploidb_config['ploidb_mysql']['username']
	db_password = ploidb_config['ploidb_mysql']['password']
	...
	if context.UserFlags_dict['MSA_Software'] == 'ClustalOmega':
		guidance_command = 'perl %s --seqFile %s ' \
						   '--msaProgram CLUSTALO --clustalo %s --seqType nuc --outDir %s --dataset %s --program GUIDANCE --TreeAlg FastTree --Tree_Param \\\\-fastest' % (
							   ploidb_config['diff_soft']['Guidance'],fasta_filename, ploidb_config['diff_soft']['ClastaO'], output_dir, dataset)
	else:
		guidance_command = 'perl %s --program GUIDANCE --seqFile %s ' \
						   '--msaProgram MAFFT --seqType nuc --outDir %s --dataset %s --MSA_Param "\-\-adjustdirection"' % (
							ploidb_config['diff_soft']['Guidance'],fasta_filename, output_dir, dataset)
...
def run_msa_clastal(local_dir, fasta_filename, fasta_final_output):
	MSA_cmd = '%s -i %s -o %s --outfmt=fasta' % (ploidb_config['diff_soft']['ClastaO'],fasta_filename, fasta_final_output)
...
def create_LocusTree_diffMethods(cluster_id, Tree_Method, context):
	...
	#scripts_dir =  ploidb_config['general']['OTT_MAIN'] + 'ott_scripts/'
	#r_path = ploidb_config['diff_soft']['R_PATH']
	#createPdfTree_Rcmd = (r_path + " CMD BATCH '--args working_dir=" + '"' + context.summary_dir + '"' +
	#					  " tree=" + '"' + final_tree + '"' + " output=" + '"' + final_tree_pdf + '"' + "' " +
	#					  scripts_dir + "plot_pdf.R")
...
def statusFail_LogFile(context, status_line):
	...
	with open(context.final_status, 'a') as f_status:
		f_status.write("Failed - %s \n" % status_line)
	#status_line.replace(' - STOPPING','')
	#status_line_noSpace=status_line.replace(' ','_')
	#f_end_status_name = context.summary_dir + '/' + status_line_noSpace
	#f_end=open(f_end_status_name,'w')
	#f_end.close()
	return
...
def generate_taxa_tree(id, taxa_list, working_dir, log_filename, debug_filename, cluster_method=None,
	...
	init_ploidb_config(args.config_filename)
	logger.info("ploidb_config=%s" % ploidb_config)
	cluster_script_dir = ploidb_config['general']['OTT_MAIN']
	...
	#Stand alone version :
	context.standAlone_Flag = ploidb_config['general']['SA_VERSION']
	#Data validation:
	if 'Outgroup_User' in context.UserFlags_dict:
		if context.UserFlags_dict['Outgroup_Flag'] == 'User':
	...
	db_schema_filename = cluster_script_dir + "/genusSchema.sql"
	...
	db_name = ploidb_config['general']['DB_DIR'] + ploidb_config['concat_headir']['NCBI_NAMES_NR']
	if 'Outgroup_User' in context.UserFlags_dict:
		if context.UserFlags_dict['Outgroup_Flag'] == 'User':
	...
	max_number_of_species = int(ploidb_config['general']['max_number_of_species'])
...
#
	...
	for cluster_file in all_fasta_file_list:
		...
		if diff_taxa_in_cluster < int(ploidb_config['general']['min_species_in_cluster']):
