...
ploidb_config = configparser.ConfigParser()
...
def init_ploidb_config(config_filename):
	global ploidb_config
	logger.info("Reading ploidb config from %s" % config_filename)
	ploidb_config.read(config_filename)
...
def getSpeciesSequenceFileLocation(taxonId):
	seqPerSpeciesDir = ploidb_config['general']['seq_per_species_dir']
...
def get_indexed_seq_hash(genbank_files_dir):
	...
	for grp_name in GROUP_LIST:
		...
		#genbank_files_dir = ploidb_config['general'][genbankDir + grp_name + '/']
		if not files:
	...
	indexFilename = ploidb_config['general']['index_Filename']
...
def use_cache_results_if_exists(file_for_hash, file_to_check_in_cache, genus_name, cache_prefix):
	...
	cached_filename = os.path.join(ploidb_config['general']['cache_path'], genus_name)
...
def cache_file(file_for_hash, source_to_cache, genus_name, cache_prefix):
	...
	cached_dir = os.path.join(ploidb_config['general']['cache_path'], genus_name)
...
def runTrimal(input_fasta,Trimal_cf):
	...
	#Now run Trimal:
	Triml_path = ploidb_config['diff_soft']['TrimlPath']
